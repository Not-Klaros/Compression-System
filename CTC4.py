...
MAGIC = b"CTC4"  # Carlos Text Compressor v4: context Huffman + token/byte fallback
FORMAT_VERSION = 4
DEFAULT_MODEL_VERSION = 1

# File header layout:
# magic:          4 bytes  b"CTC4"
...
def context_weight(token, context):
    """Bias the global model toward tokens likely after the previous-token context."""
    ...
    if context in {"WORD", "CAP_WORD", "NUMBER"}:
...
def build_context_huffman_codes(model):
    """Build several deterministic Huffman tables from one saved global model."""
    contexts = ["START", "WORD", "CAP_WORD", "NUMBER", "SPACE", "PUNCT"]
    id_to_context_code = {}
    context_code_to_id = {}
...
def unit_to_known_literal_bits(unit, model, context="START"):
    ...
    return model["id_to_context_code"].get(context, model["id_to_context_code"]["START"])[unit]
...
def compress_payload_lz77(
    ...
    for symbol in symbols:
        ...
        if kind == "LIT":
            ...
            if isinstance(unit, int):
                context = token_context(decoded_so_far[-1] if decoded_so_far else None)
                bits.append("0")
    ...
    while index < len(bits):
        ...
        if marker == "0":
            ...
            code_to_id = model["context_code_to_id"].get(
                context, model["context_code_to_id"]["START"]
            )
...
def model_info(model_db=MODEL_DB):
    ...
    return {
        ...
        "entropy_coding": "context-dependent Huffman literals",
        "payload_modes": "0=token_context_lz77_huffman, 1=byte_fallback_lz77",
    }
