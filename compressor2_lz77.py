...
MAGIC = b"CTC2"  # Carlos Text Compressor v2, token LZ77 + Huffman
FORMAT_VERSION = 2
DEFAULT_MODEL_VERSION = 1

# File header layout:
# magic:          4 bytes  b"CTC2"
...
# The compressor searches repeated token phrases and emits:
#   0 + Huffman literal
#   1 + distance + length
DISTANCE_BITS = 12
LENGTH_BITS = 8
...
DEFAULT_WINDOW_SIZE = MAX_DISTANCE
DEFAULT_MIN_MATCH = 4
DEFAULT_MAX_MATCH = MAX_MATCH_LENGTH
...
def bits_to_int(bits, index, width):
    if index + width > len(bits):
        raise ValueError("Compressed data ended while reading an integer field")
    return int(bits[index:index + width], 2), index + width
...
def unit_to_literal_bits(unit, model):
    ...
    tag, raw = unit
    if tag != "RAW":
        raise ValueError(f"Unknown unit type: {tag}")

    return escape_code + raw_to_bits(raw)
...
def encode_lz77_symbols(
    ...
    if window_size > MAX_DISTANCE:
        raise ValueError(f"window_size cannot exceed {MAX_DISTANCE}")

    if max_match > MAX_MATCH_LENGTH:
        raise ValueError(f"max_match cannot exceed {MAX_MATCH_LENGTH}")
...
def compress_payload_lz77(
    ...
    for symbol in symbols:
        ...
        if kind == "LIT":
            bits.append("0")
            bits.append(unit_to_literal_bits(symbol[1], model))

        elif kind == "MATCH":
            _, distance, length = symbol
            bits.append("1")
            bits.append(int_to_bits(distance, DISTANCE_BITS))
            bits.append(int_to_bits(length, LENGTH_BITS))
...
def decompress_payload_lz77(data, model):
    ...
    while index < len(bits):
        ...
        if marker == "0":
            ...
            distance, index = bits_to_int(bits, index, DISTANCE_BITS)
            length, index = bits_to_int(bits, index, LENGTH_BITS)
            symbols.append(("MATCH", distance, length))
...
def model_info(model_db=MODEL_DB):
    ...
    return {
        ...
        "lz77_distance_bits": DISTANCE_BITS,
        "lz77_length_bits": LENGTH_BITS,
