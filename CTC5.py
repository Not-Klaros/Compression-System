...
MAGIC = b"CTC5"  # Carlos Text Compressor v5: richer contexts + transforms + stronger match finder
FORMAT_VERSION = 5
...
def compress_payload(text, model, encoding="utf-8", **lz77_options):
    ...
    if len(byte_payload) < len(token_payload):
        return bytes([BYTE_MODE]) + byte_payload
...
def decompress_file(input_file, output_file, model_db=MODEL_DB, encoding="utf-8"):
    ...
    text = decompress_payload(compressed["payload"], model, encoding=encoding)
...
def model_info(model_db=MODEL_DB):
    ...
    return {
        ...
        "payload_modes": "0=token_context_lz77_huffman_transforms, 1=byte_fallback_lz77",
    }
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="CTC5 global-model text compressor with multi-symbol context Huffman, transforms, and stronger LZ77"
    )
