...
MAGIC = b"CTC6"  # Carlos Text Compressor v6: text model + binary/image-safe payloads
FORMAT_VERSION = 6
DEFAULT_MODEL_VERSION = 1

# File header layout:
# magic:          4 bytes  b"CTC5"
...
# Payload starts with a mode byte:
#   0x00 = token LZ77 + context-dependent Huffman literals
#   0x01 = byte-level fallback LZ77 for data that tokenizes poorly
#
# Token-mode symbol grammar after the mode byte:
#   0  + context-Huffman known-token literal
#   10 + gamma(byte_length) + raw UTF-8 bytes
#   11 + gamma(distance) + gamma(length)
#
# Byte-mode symbol grammar after the mode byte:
#   0  + raw byte
#   1  + gamma(distance) + gamma(length)
#
...
def token_context(token):
    """
    Small deterministic context bucket used for context-dependent entropy coding.

    The encoder and decoder can both derive this from the previous decoded token,
    so no extra side table has to be stored in the compressed file.
    """
    if token is None:
        return "START"
...
TOKEN_CONTEXTS = ["START", "WORD", "CAP_WORD", "NUMBER", "SPACE", "PUNCT"]


def multi_symbol_context(previous_tokens):
    """
    Deterministic 2-token context.

    CTC5 used only the previous token class. CTC5 uses the previous two token
    classes where possible. This gives separate models for patterns like:
        WORD + SPACE -> likely word
        WORD + PUNCT -> likely space
        START + START -> likely sentence opener
    """
    if not previous_tokens:
        return "START|START"

    prev_1 = token_context(previous_tokens[-1])
    prev_2 = token_context(previous_tokens[-2]) if len(previous_tokens) >= 2 else "START"
    return f"{prev_2}|{prev_1}"


def context_weight(token, context):
    """Bias the global model toward tokens likely after a 2-symbol context."""
    current = token_context(token)
    if "|" in context:
        prev_2, prev_1 = context.split("|", 1)
    else:
        prev_2, prev_1 = "START", context

    weight = 1

    if prev_1 == "START":
        if current in {"CAP_WORD", "WORD", "NUMBER"}:
            weight *= 4
        elif current == "SPACE":
            weight *= 1

    elif prev_1 in {"WORD", "CAP_WORD", "NUMBER"}:
        if current in {"SPACE", "PUNCT"}:
            weight *= 5
        elif current in {"WORD", "CAP_WORD", "NUMBER"}:
            weight *= 1
        else:
            weight *= 2

    elif prev_1 == "SPACE":
        if current in {"WORD", "CAP_WORD", "NUMBER"}:
            weight *= 5
        elif current == "SPACE":
            weight *= 1
    ...
    # Small second-order adjustments. Keep these conservative so tables do not
    # become too extreme from a model that only stores global frequencies.
    if prev_2 in {"WORD", "CAP_WORD", "NUMBER"} and prev_1 == "SPACE":
        if current in {"WORD", "CAP_WORD", "NUMBER"}:
            weight *= 2

    if prev_2 in {"WORD", "CAP_WORD", "NUMBER"} and prev_1 == "PUNCT":
        if current == "SPACE":
            weight *= 2

    if prev_2 == "PUNCT" and prev_1 == "SPACE":
        if current in {"CAP_WORD", "WORD", "NUMBER"}:
            weight *= 2
...
def build_context_huffman_codes(model):
    """Build deterministic Huffman tables for every previous-2-token context."""
    contexts = [f"{a}|{b}" for a in TOKEN_CONTEXTS for b in TOKEN_CONTEXTS]
    id_to_context_code = {}
    context_code_to_id = {}

    for context in contexts:
        adjusted = {}
        for token_id, frequency in model["frequencies"].items():
            token = model["id_to_token"].get(token_id, "")
            adjusted[token_id] = max(1, int(frequency)) * context_weight(token, context)

        codes = build_huffman_codes(adjusted)
        id_to_context_code[context] = codes
        context_code_to_id[context] = {code: token_id for token_id, code in codes.items()}

    return id_to_context_code, context_code_to_id
...
def rebuild_codes(model):
    ...
    (
        model["id_to_context_code"],
        model["context_code_to_id"],
    ) = build_context_huffman_codes(model)
...
# ============================================================
# BROTLI-STYLE STATIC DICTIONARY TRANSFORMS
# ============================================================

# These are deliberately simple and deterministic. They let an unknown word be
# represented as a known dictionary/model token plus a tiny transform ID instead
# of raw UTF-8 bytes. This is the same broad idea as Brotli's static dictionary
# transforms, but much smaller and easier to inspect.
STATIC_TRANSFORMS = [
    ("IDENTITY", "", "", "none"),
...
def apply_case_transform(text, case_mode):
...
def apply_static_transform(base_token, transform_id):
    _, prefix, suffix, case_mode = STATIC_TRANSFORMS[transform_id]
    return apply_case_transform(prefix + base_token + suffix, case_mode)


def find_static_transform(token, model):
    """Return (transform_id, base_token_id) if token can be cheaply transformed."""
    ...
    # Prefer exact lowercase identity/case transforms first.
    for transform_id, (_, prefix, suffix, case_mode) in enumerate(STATIC_TRANSFORMS):
        if prefix or suffix:
            continue
        if case_mode == "none":
            base = token
        elif case_mode == "capitalize" and token[:1].isupper() and token[1:] == token[1:].lower():
            base = lower
        elif case_mode == "upper" and token.isupper():
            base = lower
        else:
            continue
        base_id = token_to_id.get(base)
        if base_id is not None and apply_static_transform(base, transform_id) == token:
            return transform_id, base_id

    # Prefix/suffix transforms use lowercase base words. Preserve traditional
    # word forms only; odd mixed-case words fall back to raw bytes.
    if token != lower:
        return None
...
def text_to_units(text, model):
    ...
    Unknown tokens may become ("TRANSFORM", transform_id, base_token_id, token)
    if a Brotli-style transform can recreate them. Remaining unknown tokens
    become ("RAW", token) units.
    """
    units = []
    ...
    for token in tokenize(text):
        ...
        transformed = find_static_transform(token, model)
        if transformed is not None:
            transform_id, base_id = transformed
            units.append(("TRANSFORM", transform_id, base_id, token))
        else:
...
def unit_to_known_literal_bits(unit, model, context="START"):
    ...
    return model["id_to_context_code"].get(context, model["id_to_context_code"]["START|START"])[unit]
...
def find_lz77_match(units, position, recent_positions, window_size, min_match, max_match, max_chain=64):
...
def add_lz77_index_entries(units, start, end, recent_positions, min_match):
    ...
    for pos in range(start, end):
        ...
        chain = recent_positions[key]
        chain.append(pos)
        if len(chain) > 256:
            del chain[:len(chain) - 256]
        ...
        if kind == "LIT":
            ...
            if isinstance(unit, int):
                ...
                if tag == "RAW":
                    ...
                    _, transform_id, base_id, _original = unit
                    output.append(apply_static_transform(model["id_to_token"][base_id], transform_id))
                else:
...
def find_byte_lz77_match(data, position, recent_positions, window_size, min_match, max_match):
    if position + min_match > len(data):
        return None

    key = bytes(data[position:position + min_match])
    candidates = recent_positions.get(key)
...
def add_byte_lz77_index_entries(data, start, end, recent_positions, min_match):
    max_start = len(data) - min_match
...
def compress_payload_byte_fallback(
    text,
    encoding="utf-8",
    window_size=DEFAULT_WINDOW_SIZE,
    min_match=3,
    ...
    """Byte-level fallback for binary-ish, random, or badly tokenized text."""
    ...
    while position < len(data):
        ...
        if match is None:
            bits.append("0")
            bits.append(f"{data[position]:08b}")
            add_byte_lz77_index_entries(data, position, position + 1, recent_positions, min_match)
            position += 1
            continue
        ...
        add_byte_lz77_index_entries(data, position, position + length, recent_positions, min_match)
        position += length

    return bits_to_bytes("".join(bits))


def decompress_payload_byte_fallback(data, encoding="utf-8"):
    ...
    while index < len(bits):
        ...
        if marker == "0":
            if index + 8 > len(bits):
                raise ValueError("Compressed byte payload ended inside a literal byte")
            output.append(int(bits[index:index + 8], 2))
            index += 8
            ...
            if distance <= 0 or distance > len(output):
                raise ValueError("Invalid byte LZ77 match distance")
            if length <= 0:
                raise ValueError("Invalid byte LZ77 match length")

            for _ in range(length):
                output.append(output[-distance])

        else:
            raise ValueError(f"Invalid byte fallback marker bit: {marker}")

    return bytes(output).decode(encoding)


def compress_raw_bytes_lz77(
    data,
    window_size=DEFAULT_WINDOW_SIZE,
    min_match=3,
    max_match=DEFAULT_MAX_MATCH,
):
    """Binary-safe byte LZ77. Accepts bytes and returns compressed bytes."""
    if not isinstance(data, (bytes, bytearray)):
        raise TypeError("compress_raw_bytes_lz77 expects bytes-like input")
...
def decompress_raw_bytes_lz77(data):
    """Inverse of compress_raw_bytes_lz77. Returns raw bytes."""
    bits = bytes_to_bits(data)
    output = bytearray()
    index = 0
    ...
    while index < len(bits):
        ...
        if marker == "0":
            if index + 8 > len(bits):
                raise ValueError("Compressed binary payload ended inside a literal byte")
            output.append(int(bits[index:index + 8], 2))
            index += 8
            ...
            if distance <= 0 or distance > len(output):
                raise ValueError("Invalid binary LZ77 match distance")
            if length <= 0:
                raise ValueError("Invalid binary LZ77 match length")

            for _ in range(length):
                output.append(output[-distance])

        else:
            raise ValueError(f"Invalid binary fallback marker bit: {marker}")

    return bytes(output)


def compress_binary_payload(data, **lz77_options):
    """Compress arbitrary bytes. Stores uncompressed when LZ77 is not beneficial.

    Most PNG/JPEG/WebP files are already compressed, so forcing another LZ77
    pass often makes them larger. CTC6 keeps those files valid by storing them
    raw when that is smaller than the binary LZ77 representation.
    """
    lz77_payload = compress_raw_bytes_lz77(data, **lz77_options)

    if len(lz77_payload) < len(data):
        return bytes([BINARY_LZ77_MODE]) + lz77_payload

    return bytes([STORED_BINARY_MODE]) + bytes(data)


def decompress_binary_payload(mode, body):
    if mode == BINARY_LZ77_MODE:
        return decompress_raw_bytes_lz77(body)

    if mode == STORED_BINARY_MODE:
        return bytes(body)

    raise ValueError(f"Unknown binary payload mode byte: {mode}")
...
def compress_payload_lz77(
    ...
    for symbol in symbols:
        ...
        if kind == "LIT":
            ...
            if isinstance(unit, int):
                context = multi_symbol_context(decoded_so_far)
                bits.append("0")
                bits.append(unit_to_known_literal_bits(unit, model, context=context))
                decoded_so_far.append(model["id_to_token"][unit])
            else:
                tag = unit[0]
                if tag == "TRANSFORM":
                    _, transform_id, base_id, original = unit
                    context = multi_symbol_context(decoded_so_far)
                    bits.append("100")
                    bits.append(gamma_to_bits(transform_id + 1))
                    bits.append(unit_to_known_literal_bits(base_id, model, context=context))
                    decoded_so_far.append(original)
                elif tag == "RAW":
                    raw = unit[1]
                    bits.append("101")
                    bits.append(raw_to_bits(raw))
                    decoded_so_far.append(raw)
                else:
                    raise ValueError(f"Unknown literal unit type: {tag}")
            ...
            if distance <= 0 or distance > len(decoded_so_far):
                raise ValueError("Invalid LZ77 match while encoding")
            for _ in range(length):
                decoded_so_far.append(decoded_so_far[-distance])
...
def decompress_payload_lz77(data, model):
    ...
    while index < len(bits):
        ...
        if marker == "0":
            current = ""
            context = multi_symbol_context(decoded_so_far)
            code_to_id = model["context_code_to_id"].get(
                context, model["context_code_to_id"]["START|START"]
            )
            ...
            while index < len(bits):
                ...
                if current in code_to_id:
                    token_id = code_to_id[current]
                    ...
                    decoded_so_far.append(token)
                    break
            else:
                raise ValueError("Compressed data ended with incomplete context-Huffman literal")
            ...
            if extended_marker == "0":
                if index >= len(bits):
                    raise ValueError("Compressed data ended inside transformed/raw marker")
                transform_marker = bits[index]
                index += 1

                if transform_marker == "0":
                    transform_id_plus_one, index = bits_to_gamma(bits, index)
                    transform_id = transform_id_plus_one - 1
                    if transform_id < 0 or transform_id >= len(STATIC_TRANSFORMS):
                        raise ValueError("Invalid static transform id")
                    ...
                    while index < len(bits):
                        ...
                        if current in code_to_id:
                            base_id = code_to_id[current]
                            base_token = model["id_to_token"][base_id]
                            token = apply_static_transform(base_token, transform_id)
                            symbols.append(("LIT", ("TRANSFORM", transform_id, base_id, token)))
                            decoded_so_far.append(token)
                            break
                    else:
                        raise ValueError("Compressed data ended with incomplete transform base token")

                elif transform_marker == "1":
                    raw, index = bits_to_raw(bits, index)
                    symbols.append(("LIT", ("RAW", raw)))
                    decoded_so_far.append(raw)
                else:
                    raise ValueError(f"Invalid transform/raw marker bit: {transform_marker}")
...
# Default compressor tries context-token mode first, then byte fallback, and stores
# the smaller payload. The leading mode byte makes decompression deterministic.
def compress_payload(text, model, encoding="utf-8", **lz77_options):
    token_payload = compress_payload_lz77(text, model, **lz77_options)
    byte_payload = compress_payload_byte_fallback(text, encoding=encoding, **lz77_options)

    if len(byte_payload) < len(token_payload):
        return bytes([TEXT_BYTE_MODE]) + byte_payload

    return bytes([TOKEN_MODE]) + token_payload


def decompress_payload(data, model, encoding="utf-8"):
    if not data:
    ...
    if mode == TOKEN_MODE:
        if model is None:
            raise ValueError("Token payload requires a loaded text model")
        return decompress_payload_lz77(body, model)

    if mode == TEXT_BYTE_MODE:
        return decompress_payload_byte_fallback(body, encoding=encoding)

    if mode in {BINARY_LZ77_MODE, STORED_BINARY_MODE}:
        return decompress_binary_payload(mode, body)

    raise ValueError(f"Unknown payload mode byte: {mode}")
...
def looks_like_image_or_binary(path):
...
def compress_file(
    ...
    use_binary = input_type == "binary" or (input_type == "auto" and looks_like_image_or_binary(input_file))
    text = None
    ...
    if use_binary:
        payload = compress_binary_payload(
            raw,
            window_size=window_size,
            min_match=max(3, min_match),
            max_match=max_match,
        )
        save_compressed_file(payload, output_file, 0)
        return compression_stats_bytes(raw, payload, include_header=True)
...
def decompress_file(input_file, output_file, model_db=MODEL_DB, encoding="utf-8"):
    ...
    if mode in {BINARY_LZ77_MODE, STORED_BINARY_MODE}:
        raw = decompress_payload(payload, None, encoding=encoding)
    ...
    text = decompress_payload(payload, model, encoding=encoding)
...
def compression_stats_bytes(raw, payload, include_header=False):
...
def compression_stats(text, payload, include_header=False):
    return compression_stats_bytes(text.encode("utf-8"), payload, include_header=include_header)
...
def compare_methods(text, model, include_header=False):
    ...
    byte_payload = compress_payload_byte_fallback(text)
    auto_payload = compress_payload(text, model)

    return {
        "huffman_only": compression_stats(text, huffman_payload, include_header=include_header),
        "context_lz77_huffman": compression_stats(text, lz77_payload, include_header=include_header),
        "byte_fallback_lz77": compression_stats(text, byte_payload, include_header=include_header),
        "auto_best_with_mode": compression_stats(text, auto_payload, include_header=include_header),
    }
...
def model_info(model_db=MODEL_DB):
    ...
    return {
        ...
        "entropy_coding": "multi-symbol context Huffman literals",
        "payload_modes": "0=token_context_lz77_huffman_transforms, 1=text_byte_lz77, 2=binary_lz77, 3=stored_binary",
    }
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="CTC6 text + image-safe compressor with binary payload support"
    )
    ...
    comp.add_argument("--input-type", choices=["auto", "text", "binary"], default="auto",
                      help="auto detects common image extensions/binary files; binary is safest for images")
...
def main(argv=None):
    ...
    if args.command == "train":
        ...
        print_stats("Context Token LZ77 + Huffman:", results["context_lz77_huffman"])
        print_stats("Byte fallback LZ77:", results["byte_fallback_lz77"])
        print_stats("Auto selected payload:", results["auto_best_with_mode"])
